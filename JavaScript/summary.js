...
async function sumOfTask() {
    ...
    try {
        ...
        for (const key in data) {
            if (data[key].Status === "To-Do") { // Großschreibung und Bindestrich beachten
                todoCount++;
            }
        }

        // Element korrekt updaten
        const todoElement = document.querySelector('.card-number');
        if (todoElement) {
            todoElement.textContent = todoCount;
        }

    } catch (error) {
        console.error("Fehler beim Abrufen der To-Do Tasks:", error);
    }
}
